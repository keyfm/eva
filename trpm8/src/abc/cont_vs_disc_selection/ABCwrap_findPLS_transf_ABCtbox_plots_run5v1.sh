...
nums=$(zcat /mnt/scratch/felix/trpm8/abc/run${rid}_out/all_sumstat_5ParamBT_${mod}_eur_sdn_wTrunc.tsv.gz |head -n1 |awk -v first=${ss} '{for(i=1;i<=NF;i++){if($i=="ihsP1"){ihsPOS=i};if($i==first){ssPOS=i};}print ihsPOS,ssPOS,ssPOS-ihsPOS+1,ssPOS-ihsPOS+5,ssPOS-ihsPOS+5+1,ssPOS-ihsPOS+5+5}')
...
for m in ntr sdn ssv; do echo $m
 for t in nTrunc wTrunc; do echo $t
 for c in eur asi; do echo $c
  zcat /mnt/scratch/felix/trpm8/abc/run${rid}_out/all_sumstat_5ParamBT_${mod}_${c}_${m}_${t}.tsv.gz > /tmp/fmk_sims_${rid}_v${v}.tsv
  /home/felix_schulze/src/ABCtoolbox/binaries/linux/transformer \
  /mnt/scratch/felix/trpm8/abc/analysis/run${rid}/version${v}/find_pls_${c}_allMod_v${v}.plsda \
  /tmp/fmk_sims_${rid}_v${v}.tsv \
  /mnt/scratch/felix/trpm8/abc/run${rid}_out/transf/all_sumstat_5ParamBT_trans_${c}_${m}_${t}_run${rid}v${v}.tsv \
  boxcox
  rm /tmp/fmk_sims_${rid}_v${v}.tsv
 ## take only PLS 1-5 for inference (to actually reduce the stats!) and bgzip full file
 sed -e 's/^[ \t]*//' /mnt/scratch/felix/trpm8/abc/run${rid}_out/transf/all_sumstat_5ParamBT_trans_${c}_${m}_${t}_run${rid}v${v}.tsv |cut -f${cutSims} > /mnt/scratch/felix/trpm8/abc/run${rid}_out/transf/all_sumstat_5ParamBT_trans_${c}_${m}_${t}_run${rid}v${v}_5PLS.tsv
 bgzip -f /mnt/scratch/felix/trpm8/abc/run${rid}_out/transf/all_sumstat_5ParamBT_trans_${c}_${m}_${t}_run${rid}v${v}.tsv
 done; done
done
...
for c in eur asi; do echo $c
 ...
 for p in ${pops}; do echo $p
  ...
  for t in nTrunc wTrunc; do echo $t
   ...
   |sed -e "s/-CONT-/${c}/g" -e "s/-MODEL-/${m}/g" -e "s/-POP1-/YRI/" -e "s/-POP2-/${p}/g" -e "s/-PARAM_COLS-/${para_cols}/" -e "s/-RID-/${rid}/g" -e "s/-VERSION-/${v}/g" -e "s/-TRUNCATION-/${t}/g" \
   > /mnt/scratch/felix/trpm8/abc/analysis/run${rid}/version${v}/YRI_${p}/ABCest_YRI_${p}_${m}_${t}.input
   /mnt/scratch/felix/trpm8/abc/analysis/run${rid}/ABCtoolbox /mnt/scratch/felix/trpm8/abc/analysis/run${rid}/version${v}/YRI_${p}/ABCest_YRI_${p}_${m}_${t}.input > /mnt/scratch/felix/trpm8/abc/analysis/run${rid}/version${v}/YRI_${p}/std_errout_${m}_${t}.txt 2>&1
  done; done
 done
done
...
echo -e "Merge Tables."
for p in CEU GBR TSI FIN IBS CDX CHB CHS KHV JPT BEB GIH ITU PJL STU; do
 cd /mnt/scratch/felix/trpm8/abc/analysis/run${rid}/version${v}/YRI_${p}/
 paste <(echo $p) <(cut -f2 out_ntr_nTrunc.marginalDensity.txt) <(cut -f2 out_ntr_wTrunc.marginalDensity.txt) <(cut -f2 out_sdn_nTrunc.marginalDensity.txt) <(cut -f2 out_sdn_wTrunc.marginalDensity.txt) <(cut -f2 out_ssv_nTrunc.marginalDensity.txt) <(cut -f2 out_ssv_wTrunc.marginalDensity.txt)
done > /mnt/scratch/felix/trpm8/abc/analysis/run${rid}/version${v}/marginalDensities_ntr_sdn_ssv_nTwT.txt

############################################################
############ 4. Plotting ModelChoice and PostEstimateDensities
############################################################
Rscript /mnt/scratch/felix/trpm8/abc/abc_scripts/ABCplot_PostSelMod_PostParam_PLS5clouds_wVctrl1_wTnT.r ${rid} ${v} ${pps}
