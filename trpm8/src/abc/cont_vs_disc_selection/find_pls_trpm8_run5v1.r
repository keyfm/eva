...
filename1 <- paste("all_sumstat_5ParamBT_",mod,"_",c,"_sdn_nTrunc.tsv.gz",sep="");

filename2 <- paste("all_sumstat_5ParamBT_",mod,"_",c,"_ssv_nTrunc.tsv.gz",sep="");

filename3 <- paste("all_sumstat_5ParamBT_",mod,"_",c,"_ntr_nTrunc.tsv.gz",sep=""); # n Trunc and wTrunc same. Arbitrary choice

filename4 <- paste("all_sumstat_5ParamBT_",mod,"_",c,"_sdn_wTrunc.tsv.gz",sep="");

filename5 <- paste("all_sumstat_5ParamBT_",mod,"_",c,"_ssv_wTrunc.tsv.gz",sep="");
...
model <- as.factor(rep(0:4,each=nr))
...
sdnT <- read.table(gzfile(paste(directory, filename4, sep="")), header=T, nrows=nr, skip=0); # aka 'a'
ssvT <- read.table(gzfile(paste(directory, filename5, sep="")), header=T, nrows=nr, skip=0); # aka 'b'

dt <- rbind(sdn,ssv,ntr,sdnT,ssvT)
stats <- dt[,statCols]
params <- dt[,parCols]
