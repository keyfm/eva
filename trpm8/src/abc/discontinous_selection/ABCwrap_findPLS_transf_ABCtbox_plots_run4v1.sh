...
nums=$(zcat /mnt/scratch/felix/trpm8/abc/run${rid}_out/all_sumstat_5ParamBT_${mod}_eur_sdn.tsv.gz |head -n1 |awk -v first=${ss} '{for(i=1;i<=NF;i++){if($i=="ihsP1"){ihsPOS=i};if($i==first){ssPOS=i};}print ihsPOS,ssPOS,ssPOS-ihsPOS+1,ssPOS-ihsPOS+5,ssPOS-ihsPOS+5+1,ssPOS-ihsPOS+5+5}')
